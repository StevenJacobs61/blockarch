...
export const getProjectsByUserId = async (user) => {
    const response = await axios.post(`${process.env.REACT_APP_BACKEND_BASE_URL}/user-project/projects`, user);
    return response.data;
}
export const deleteProject = async (project, results) => {
    ...
    try {
        ...
        const networkRes = await deleteById(industryId, '/user-project-network-participants');
        const langaugesRes = await deleteById(industryId, '/user-project-languages');
        ...
        results.map(async(result) => {
            try {
                const resultRes = await updateById({...result, user_project_id:null}, '/project-blockchain-result', result.id);
                const deleteResultRes = await deleteById(result.id, '/project-blockchain-result');
            } catch (error) {
                console.error(error);
                success = false;
            }
